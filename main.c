...
int main() {
  ...
  while (1) {
    ...
    bolx = bolx + movex;
    boly += movey;
    // Условие касания ракетки слева/справа
    if (bolx == 23 || bolx == 2) movex = movex * -1;
    ...
    if ((boly == 2) && (bolx == p1x || bolx == p1x +1 || bolx == p1x +2)) movey = movey * -1;
    if ((boly == 77) && (bolx == p2x || bolx == p2x +1 || bolx == p2x +2)) movey = movey * -1;
    // Функция счетчика
    if (boly == 77) count_left++;
    if (boly == 2) count_right++;
    ...
    if (count_left == 21 || count_right == 21) {
      if (count_left == 21) printf("1st player won!!!\n");
      if (count_right == 21) printf("2ed player won!!!\n");
      break;
    } else {
